...
.hdr { background:#0F172A; border-radius:12px; padding:24px 28px 20px; margin-bottom:24px; }
.hdr h1 { font-size:18px; font-weight:700; color:#F1F5F9; margin:0 0 4px; }
.hdr p  { font-size:12px; color:#64748B; margin:0 0 12px; }
.hdr-tag { display:inline-block; font-size:10.5px; font-weight:600; color:#94A3B8;
           border:1px solid #334155; border-radius:20px; padding:2px 10px; margin:0 4px 4px 0; }
...
.card     { background:#fff; border:1px solid #E2E8F0; border-radius:10px; padding:16px 18px; margin-bottom:10px; }
...
.kpi { flex:1; min-width:100px; background:#F8FAFC; border:1px solid #E2E8F0; border-radius:8px; padding:12px 14px; }
...
.type-box { flex:1; min-width:100px; background:#F8FAFC; border:1px solid #E2E8F0; border-radius:8px; padding:12px 10px; text-align:center; }
...
.score-chip { font-size:18px; font-weight:700; padding:2px 10px; border-radius:6px; }
...
.cause-box { background:#F8FAFC; border:1px solid #E2E8F0; border-radius:6px;
             padding:7px 11px; margin-bottom:10px; font-size:11.5px; color:#374151; }
.offer-chip { display:inline-block; font-size:11.5px; font-weight:600; color:#1D4ED8;
...
.alert-box { background:#FFFBEB; border:1px solid #FDE68A; border-left:3px solid #F59E0B;
...
.new-alert-box { background:#FFF7ED; border:1.5px solid #FDBA74; border-left:4px solid #EA580C;
                 border-radius:10px; padding:16px 20px; margin:16px 0; }
.new-alert-ttl { font-size:13px; font-weight:700; color:#9A3412; margin-bottom:8px; }
.new-alert-item { font-size:12px; color:#7C2D12; padding:4px 0;
                  border-bottom:1px solid #FED7AA; display:flex; justify-content:space-between; }
.new-alert-item:last-child { border-bottom:none; }

.edit-card { background:#fff; border:1.5px solid #3B82F6; border-radius:10px;
             padding:16px 18px; margin-bottom:10px; }
.edit-head { display:flex; justify-content:space-between; align-items:center; margin-bottom:10px; }

.result-box { background:#0F172A; border-radius:10px; padding:20px 24px; margin-top:12px; }
...
.quote-box { background:#EFF6FF; border-left:3px solid #3B82F6; border-radius:0 8px 8px 0;
             padding:12px 18px; margin-top:12px; font-size:12.5px; color:#1E40AF;
...
def detect_new_combos(high_df, threshold=15):
    """
    신규 변수 조합 Alert:
    상위 기여 변수 2개 조합이 이전에 없던 패턴인지 감지
    동일 조합이 threshold건 이상 누적되면 Alert
    """
    combo_counts = {}
    combo_examples = {}
...
def render_customer_card(r, edit_mode=False, idx=0):
    ...
    if edit_mode:
        with st.container():
            ...
            with col_exc:
                exclude = st.checkbox("제외", key=key_exc)
            if exclude:
                st.caption(f"🚫 {r['고객ID']} — 캠페인 제외")
            else:
...
<div class="hdr">
  ...
  <span class="hdr-tag">신규 패턴 Alert</span>
</div>
""", unsafe_allow_html=True)

# ══ STEP 1 ══
st.markdown('<p class="sec-tag">STEP 01</p><p class="sec-head">고객 데이터 업로드</p>', unsafe_allow_html=True)
uploaded = st.file_uploader("CSV 또는 Excel (고객번호, Var1~Var18, Score)", type=["csv","xlsx"])
...
if df is not None:
    ...
    with col_sl:
        threshold = st.slider(
            "이탈 위험 임계값 (Score 기준 — 이 값 이상 고위험 분류)",
            min_value=0.0, max_value=1.0, value=0.1, step=0.01,
            format="%.2f"
        )
        alert_min = st.slider(
            "신규 변수 조합 Alert 기준 (해당 조합이 N건 이상 누적 시 Alert)",
    ...
    if run or "done" in st.session_state:
        ...
        st.markdown(
            ...
            f'<div class="kpi"><div class="kpi-v r">{len(high)}</div><div class="kpi-l">고위험 고객<br>(Score ≥ {thr:.2f})</div></div>'
        ...
        if len(high)>0:
            ...
            for tp,cnt in sorted(td.items(),key=lambda x:x[1],reverse=True):
                ...
                html += (f'<div class="type-box"><div class="ico">{TYPE_ICONS.get(tp,"")}</div>'
                         f'<div class="nm">{tp}</div><div class="cnt">{cnt}명</div>'
                         f'<div class="str">{s}</div></div>')
        ...
        if len(newp)>0:
            ...
            st.markdown(
                f'<div class="alert-box"><div class="alert-ttl">⚠ 신규 복합 이탈 패턴 Alert</div>'
                f'<div class="alert-txt">단일 유형으로 분류되지 않는 복합 패턴 고객 <b>{len(newp)}명</b> 감지.'
                f' 온톨로지 신규 유형 추가를 검토해 주세요.<br>'
                f'<span style="font-size:11px">대상: {ids_str} 등</span></div></div>',
                unsafe_allow_html=True)
        ...
        if len(high) > 0:
            ...
            if new_combos:
                ...
                for combo, cnt in sorted(new_combos.items(), key=lambda x:x[1], reverse=True):
                    ...
                    combo_rows += (
                        f'<div class="new-alert-item">'
                        f'<span><b>{v1_name}</b> <span style="font-size:10px;color:#C2410C">({v1_type})</span>'
                        f' + <b>{v2_name}</b> <span style="font-size:10px;color:#C2410C">({v2_type})</span></span>'
                        f'<span style="font-size:11px;color:#9A3412;font-weight:600">{cnt}건 · 예) {ex_ids}</span>'
                        f'</div>'
                    )
                st.markdown(
                    f'<div class="new-alert-box">'
                    f'<div class="new-alert-ttl">🔔 신규 변수 조합 패턴 감지 — 온톨로지 업데이트 검토 필요</div>'
                    f'기존 온톨로지에 없는 변수 조합이 <b>{alert_min}건 이상</b> 반복 감지되었습니다.'
                    f' 아래 조합을 검토 후 새로운 이탈 유형으로 등록해 주세요.<br><br>'
                    f'{combo_rows}'
                    f'</div>',
                    unsafe_allow_html=True)
        ...
        with cb_:
            edit_btn = st.button("✏️  일부 수정 후 실행", use_container_width=True)
        with cc:
        ...
        if st.session_state.get("edit_mode", False):
            st.info("✏️  수정 모드 — 각 고객 카드에서 오퍼를 변경하거나 캠페인에서 제외할 수 있습니다. 완료 후 '수정 완료 및 실행'을 눌러주세요.")
        ...
        if approve or "camp" in st.session_state:
            ...
            ri = "".join(
                f'<div><div class="ri-v">{cnt}</div><div class="ri-l">{TYPE_ICONS.get(tp,"")} {tp}</div></div>'
            ...
            st.markdown(
                ...
                f'<div><div class="ri-v">{target_cnt}</div><div class="ri-l">총 발송 대상</div></div>'
                f'{ri}</div>'
            ...
            st.markdown(
                '<div class="quote-box">"이제 마케터의 역할은 데이터를 찾는 것이 아니라,<br>'
